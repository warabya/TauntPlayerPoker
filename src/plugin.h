...
PLUGINS_EXPORTDLL void ts3plugin_onVoiceRecordDataEvent(const float* data, unsigned int dataSize);
...
PLUGINS_EXPORTDLL void ts3plugin_onFileListEvent(uint64 serverConnectionHandlerID, uint64 channelID, const char* path, const char* name, uint64 size, uint64 datetime, int type);
...
PLUGINS_EXPORTDLL void ts3plugin_onServerPermissionErrorEvent(uint64 serverConnectionHandlerID, const char* errorMessage, unsigned int error, const char* returnCode, anyID failedPermissionID);
